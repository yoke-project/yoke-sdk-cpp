...
| **Planning item** | yoke-project/yoke-sdk-cpp#3 |

## yoke-sdk-cpp:planning-status.01 — the workflow runs on the work, and runs nothing of its own
...
## yoke-sdk-cpp:planning-status.02 — it is granted nothing, and with no credential it does nothing
...
## yoke-sdk-cpp:planning-status.03 — an item moves forward, and never back
...
## yoke-sdk-cpp:planning-status.04 — the items are read from the change, and never invented
...
## yoke-sdk-cpp:planning-status.05 — a query that fails moves nothing, and says so
